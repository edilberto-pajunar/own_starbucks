...
class AddCustomView extends StatefulWidget {
  const AddCustomView({super.key});

  @override
  State<AddCustomView> createState() => _AddCustomViewState();
}

class _AddCustomViewState extends State<AddCustomView> {
  final _formKey = GlobalKey<FormState>();
  final _nameController = TextEditingController();
  final _baseDrinkController = TextEditingController(text: 'Caffe Latte');

  MilkType _selectedMilk = MilkType.whole;
  SugarLevel _selectedSugar = SugarLevel.standard;
  CupSize _selectedSize = CupSize.grande;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<CustomBloc, CustomState>(
        listenWhen: (prev, curr) =>
            prev.customizeStatus != curr.customizeStatus,
        listener: (context, state) {
          if (state.customizeStatus == CustomStatus.success) {
            context.pop();
          }
        },
  ...
  Widget _buildBottomBar() {
    return BlocSelector<CustomBloc, CustomState, CustomStatus>(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: SizedBox(
                    height: 50,
                    child: customizeStatus == CustomStatus.loading
                        ? const CircularProgressIndicator()
                        : ElevatedButton(
                            onPressed: _saveCustomDrink,
